/*
    Aquarium Automation using NodeMCU
    Copyright (C) 2021 desiFIsh
...
/**
 * @brief Checks time conditions for relay operation
 * @param onTime Time to turn relay on (in 24hr format, e.g. 1435 for 2:35 PM)
 * @param offTime Time to turn relay off (in 24hr format)
 * @param number Relay number (1-4) to control
 */
void checkTimeFor(int, int, int);

/**
 * @brief Displays WiFi signal strength icon on OLED
 */
void showWifiSignal();

/**
 * @brief Displays current time and date on OLED
 */
void showTime();
...
/**
 * @brief Displays relay status indicators on OLED
 */
void relayStatusPrinter();

/**
 * @brief Handles web client connections and generates HTML response
 * @param client WiFiClient object for the connected client
 * @param currentLine Current input line from client
 */
...
/**
 * @brief Initializes relay states after power loss
 * Checks and sets initial states for all relays based on their activation settings
 */
 ...
 * Initializes hardware, connects to WiFi, starts web server and configures OTA updates
 */
void setup()
{
 ...
 * Handles OTA updates, web clients, relay timing, display updates and power save modes
 */
void loop()
{
  ArduinoOTA.handle();
